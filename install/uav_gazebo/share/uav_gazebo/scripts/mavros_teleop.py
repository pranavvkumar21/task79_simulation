...
class MavController:
    ...
    def __init__(self):
        ...
        rclpy.Subscriber("/mavros/local_position/pose", PoseStamped, self.pose_callback)
        rclpy.Subscriber("/mavros/rc/in", RCIn, self.rc_callback)

        self.cmd_pos_pub = rclpy.Publisher("/mavros/setpoint_position/local", PoseStamped, queue_size=1)
        self.cmd_vel_pub = rclpy.Publisher("/mavros/setpoint_velocity/cmd_vel_unstamped", Twist, queue_size=1)
        self.rc_override = rclpy.Publisher("/mavros/rc/override", OverrideRCIn, queue_size=1)

        # mode 0 = STABILIZE
        # mode 4 = GUIDED
        # mode 9 = LAND
        self.mode_service = rclpy.ServiceProxy('/mavros/set_mode', SetMode)
        self.arm_service = rclpy.ServiceProxy('/mavros/cmd/arming', CommandBool)
        self.takeoff_service = rclpy.ServiceProxy('/mavros/cmd/takeoff', CommandTOL)

        self.rc = RCIn()
        self.pose = Pose()
        self.timestamp = rclpy.Time()
